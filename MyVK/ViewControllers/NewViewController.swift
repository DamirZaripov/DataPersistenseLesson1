...
class NewViewController: UITableViewController, DataTransferProtocol, UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func createTestNews() {
        let testNews1 = News(id: 1, name: nameLabel.text!, surname: surnameLabel.text!, date: newsTestDateArray[0], text: nil, image: newsTestImageArray[0], numberOfLikes: newsTestLikesArray[0], numberOfComments: newsTestCommentsArray[0], numberOfReposts: newsTestRepostsArray[0], avatar: user.avatar )
        ...
        newsRepository.syncSave(with: news)
    ...
    func didPressDone(with note: String) {
        let newNews = News(id: newsId, name: nameLabel.text!, surname: surnameLabel.text!,
                            ...
                            avatar: user.avatar)
        newsId += 1
        newsRepository.asynSave(with: newNews) { [weak self] (isSaved) in
    ...
    func testSynhSearch() {
        let news = newsRepository.syncSearch(by: 3)
        print("\(String(describing: news?.text))")
    }

    func testAsynSearch() {
        newsRepository.asynSearch(by: 2) { (news) in
            print("\(String(describing: news?.text))")
        }
    }
